...
public class MessageListener extends ListenerAdapter {
	...
	public void onMessageReceived(final MessageReceivedEvent event) {
		...
		if (UserService.approvalList.containsKey(event.getAuthor().getAsTag())) {
			String transactionType = event.getMessage().getContentRaw();
			Entry<String, Boolean> entryMap = UserService.approvalList.get(event.getAuthor().getAsTag());

			if (entryMap.getKey().equals(String.valueOf(transactionType))) {
				UserService.approvalList.put(event.getAuthor().getAsTag(), new SimpleEntry<String, Boolean>(String.valueOf(transactionType), true));
				event.getAuthor().openPrivateChannel().flatMap(channel -> channel.sendMessage("Action completed")).queue();
			}
		}
	}
